...
(function() {
  ...
  // Adaptive frames: size each service photo's frame to match its real
  // aspect ratio (landscape photos get a landscape frame, portrait photos
  // get a portrait frame) — so any future upload just fits automatically.
  function applyNaturalAspect(img) {
    if (!img.naturalWidth || !img.naturalHeight) return;
    let ratio = img.naturalWidth / img.naturalHeight;
    // Clamp to sensible bounds so an unusually extreme photo doesn't break the row layout
    ratio = Math.max(0.55, Math.min(1.9, ratio));
    const frame = img.closest('.service-visual');
    if (frame) frame.style.aspectRatio = ratio.toFixed(3);
  }
  document.querySelectorAll('.service-visual img').forEach(img => {
    if (img.complete && img.naturalWidth) {
      applyNaturalAspect(img);
    } else {
      img.addEventListener('load', () => applyNaturalAspect(img));
    }
  });
